...
class _FoodInputScreenState extends State<FoodInputScreen> {
  ...
  void _searchFoods(String query) {
    if (query.isEmpty) {
      if (mounted)
        setState(() => _searchResults = FoodDatabase.foods.take(10).toList());
      return;
    }
    setState(() {
      ...
      setState(() {
        ...
        if (calories != null && calories > 0) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Could not estimate calories. Please try another term by avoiding special characters.',
              ),
            ),
          );
        }
      ...
      if (user != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Logged ${foodIntake.totalCalories.toStringAsFixed(0)} calories',
              ),
            ),
          );
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      subtitle: Text(
                        '${food.caloriesPerServing.toStringAsFixed(0)} cal per ${food.servingSize}',
                      ),
